...
pub const SIGNING_KEY_SIZE: usize = 4896;
pub const VERIFYING_KEY_SIZE: usize = 2592;
...
pub fn sign(seed: &[u8; SEED_SIZE], message: &[u8]) -> [u8; SIGNATURE_SIZE] {
    mldsa_core::sign::<MlDsa87, SIGNATURE_SIZE>(seed, message)
}

pub fn verify(vk: &[u8; VERIFYING_KEY_SIZE], message: &[u8], sig: &[u8; SIGNATURE_SIZE]) -> bool {
    mldsa_core::verify::<MlDsa87, VERIFYING_KEY_SIZE, SIGNATURE_SIZE>(vk, message, sig)
}
